...
Unlike prior works, our approach does not rely on explicit trigger information. Instead, it uses key arguments to detect the occurrence of
an event. Therefore, we represent problem as a 2-stage pipeline. This is depicted in Figure~\ref{fig:model}. The input to the pipeline is
the raw text. The outputs are the labeling sequence(s) and event type(s) -- if any event is detected.

The first stage of the pipeline identifies the key arguments in a sentence. If a sentence contains \emph{all key arguments} of a specific
event type, it will be considered to imply an event mention of this specified type. This stage outputs a labeling sequence and the
corresponding event type (if an event is detected). Since at this stage we do not concern non-key arguments, all non-key argument tokens
...
second stage of the pipeline takes in the outputs of the first stage. It then detects all the non-key arguments of the sentence and tags
the non-key argument tokens.
...
The model used in stage 1 consists of a Bidirectional Long Short-Term Memory (BLSTM) network with a conditional random field (CRF) layer
...
\paragraph{CRF}
A straightforward way to find the label sequence for a given sentence is to choose the best label for each word individually according to
the BLSTM output.
...
The output sequences of BLSTM-CRF do not necessarily satisfy the structural constraints of event extraction. For instance, regardless of
how many key arguments are correctly identified by BLSTM-CRF, if there is one key argument missing, this detection should be considered as
failed.
...
This step simply maps the input sentence to a specific event type, checking if it contains all the key arguments of that event type by
