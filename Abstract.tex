...
The effectiveness of event extractors is bound by the number and the quality of  the training instances. 
Existing  training data must be manually generated through a combination of expert
domain knowledge and extensive human involvement. However, due to drastic efforts required in annotating text, the resulted datasets are usually
small, which severally affects the quality of the learned model, making it hard to generalize.
%
Our work develops an automatic approach for generating training data for event extraction, and allows us %, for the first time,
to construct training data with hundreds of thousands of instances, % instead of just thousands, and it does this 
at a much lower cost than a manual approach. 
We achieve this %remarkable result 
by employing distant supervision to automatically create event annotations from unlabelled
...
knowledge extracted from structured knowledge bases to automatically annotate typed events with corresponding  arguments in text. 
We evaluate our approach by using the knowledge extracted from Freebase to label texts from Wikipedia articles. Experimental results show that our
