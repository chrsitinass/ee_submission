...
with a single type. In our real-world, however, there are countless examples where trigger annotations are unavailable (\FIXME{ZW: This has
nothing to do with single-token triggers??} or or one event is associated with multiple types. To make event extractors more practical, we
