...
Remarkably, our BLSTM-CRF-ILP$_{multi}$ model can find more CVT instances that are currently not referenced in Freebase. Our model detects two events in S7, while arguments of the second event (in red boxes) do not match any existing CVT instances in \FB, which do not receive any credit during automatic evaluation, but should be populated into \FB. %leading to a missing event in data generation.
